...
func (me db) GetMetric(metricSkey int) (*domain.CovidMetric, error) {
...
func (me db) GetMetricsByCategory(code *domain.Code) ([]*domain.CovidMetric, error) {
	...
	var metrics []*domain.CovidMetric
	res, err := me.configuration.dataSource.Query("SELECT * FROM metric where category_skey = ?", code.Id)
	defer res.Close()
...
func (me db) GetMetricsBySchoolAndCategory(school *domain.Code, category *domain.Code) ([]*domain.CovidMetric, error) {
	var metrics []*domain.CovidMetric
	res, err := me.configuration.dataSource.Query("SELECT * FROM metric where school_skey = ? and category_skey = ?", school.Id, category.Id)
	defer res.Close()
